...
```scss
background: linear-gradient(#fb3 50%, #58a 50%);  /* 当渐变色的色标位置重合时，就不再是渐色，而是各占一半的实色 */
```

```css
background: linear-gradient(#fb3 30%, #58a 30%); 
background-size: 100% 30px; /* 调整背景尺寸，第一个参数是宽，第二个参数是高；背景就会在容器内平铺，形成条纹，并且这是个不等高的条纹 */
```

#### 垂直条纹

```css
background: linear-gradient(to right, #fb3 30%, #58a 30%); /* 只需要添加一个指定渐变的方向属性就可 */
background-size: 30px 100%; /* 背景尺寸的属性要和水平条纹相反 */
```

#### 斜向条纹

斜向条纹和想象中的实现方式不太一样，如果是直接指定渐变方向`45deg`实现的效果会是这样的：

<img src="D:\MD_note\learingNote\img\失败斜条纹.png" alt="边框内圆角" style="zoom:35%;" />

原因是这只把每个背景贴片都旋转了45°，想要做成真正的斜条纹，需要让单个贴片包含四条条纹，从而使它旋转后可以和其它贴片条纹完美衔接，

```css
background: linear-gradient(45deg, #fb3 25%, #58a 0, #58a 50%, #fb3 0, #fb3 75%, #58a 0); /* 第二个色标位置是0，是因为当第二个色标位置比它前面的色标位置值都小的时候，第二个色标位置会被设置成它前面所有色标位置的最大值 */
background-size: 30px 30px;
```

但是上面的方法有个问题就是，如果倾斜的角度不是45°怎么办？还要考虑色标在什么位置上才能让贴片完美衔接，而`reapeating-linear-gradient`可以完美解决这个问题。它和`linear-gradient`的不同是，色标是无限循环重复直至填满整个背景的，所以它无需使用`background-size`来设置贴片的大小，只需要给容器设置`height: 100%`。

```css
background：repeating-linear-gradient(60deg, #fb3, #fb3 15px, #58a 0, #58a 30px);/* 直接在属性内设置条纹宽度 */
height: 100%;
```

当需要同色系的条纹背景时，可以直接设定一个背景色，然后把半透明白色的条纹叠加上去

``` css
background: #58a;
background-image: repeating-linear-gradient(30deg, hsla(0, 0%, 100%, .1), 
hsla(0, 0%, 100%, .1) 15px, transparent 0, transparent 30px);
/* 新版css似乎支持以下写法：background-image: repeating-linear-gradient(30deg, 
hsla(0, 0%, 100%, .1) 0 15px, transparent 0 30px); */
height: 100%;
```
